...
int main(int argc, char** argv)
{
	...
	/* もし -src が設定されていて、明示的な outdir が与えられていない場合、デフォルトの outdir をプロジェクト名とします
	* (ただし、VSX/VS6/Xcode/objc/winobjc プロジェクトの場合を除きます) */
	if (gen_src && outdir[0] == '\0' && !gen_vcxproj && !gen_vs6 && !gen_xcode && !gen_objc && !gen_winobjc)
	{
		snprintf(outdir, sizeof(outdir), "%s", proj);
	}

	/* DSPテンプレート処理（-vcxproj, -vs6, -objc, -winobjc, -xcode 以外の時のみ） */
	char* win_out = NULL;
	char* con_out = NULL;

	if (!gen_vcxproj && !gen_vs6 && !gen_objc && !gen_winobjc && !gen_xcode)
	{
	...
	/* デフォルト: DSP/DSW ファイルを出力 (除外: xcode/vcxproj/vs6/winobjc/objc) */
	if (!gen_objc && !gen_winobjc && !gen_vcxproj && !gen_vs6 && !gen_xcode)
	{
	...
	/* -src 指定時に main.c を生成（-vcxproj と -vs6 と -xcode 以外） */
	if (gen_src && !gen_vcxproj && !gen_vs6 && !gen_objc && !gen_winobjc && !gen_xcode)
	{
	...
	/* 出力メッセージ */
	/* xcode モードでは DSP/DSW は生成されないためメッセージ表示を抑制する */
	if (!gen_objc && !gen_winobjc && !gen_vcxproj && !gen_vs6 && !gen_xcode)
	{
		printf("生成しました: win_%s.dsp, con_%s.dsp, %s.dsw, build_%s.bat\n", proj, proj, proj, proj);
	}
	if (gen_src && !gen_vcxproj && !gen_vs6 && !gen_xcode) printf("生成しました: %s_main.c\n", proj);
	if (gen_vcxproj && !gen_winobjc)
	{
